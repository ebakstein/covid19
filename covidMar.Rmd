...
maxCounts <- covid %>% filter(country %in% selCountries) %>% group_by(country) %>% summarize(dateMax=Date[which.max(cases)],count=max(cases), deaths=deaths[which.max(cases)], recovered=recovered[which.max(cases)],newCases=incident[which.max(cases)]) %>% mutate(propIncrease=newCases/(count-newCases),deathRate=deaths/count) %>% ungroup()
