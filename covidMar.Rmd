...
maxCounts <- covid %>% filter(country %in% selCountries) %>% group_by(country) %>% summarize(dateMax=Date[cases==max(cases)],count=max(cases), deaths=deaths[cases==max(cases)], recovered=recovered[cases==max(cases)],newCases=incident[cases==max(cases)]) %>% mutate(propIncrease=newCases/(count-newCases),deathRate=deaths/count) %>% ungroup()
...
covidCZ[,c('date','propIncrease','newCases')]  %>% mutate(propIncrease=propIncrease*100) %>% reshape2::melt(id=c('date')) %>% ggplot(aes(x=date,y=value,col=variable)) + geom_line() + xlim(as.Date('2020-02-26'),NA) + ggtitle('COVID-19 Czech Republic: proportional increase and new cases (UZIS data)')
...
kable(covidCZ[covidCZ$date>=as.Date('2020-02-22'),])
...
pl3 
ggsave('CZvsLaggedOther.png',pl3,width=20,height=15,units='cm',scale=.8)
# pl2+facet_wrap(.~country)

pl4<-pl3 + scale_y_log10() + theme(legend.position = 'none')

pl4
ggsave('CZvsLaggedOtherLOG.png',pl4,width=20,height=15,units='cm',scale=.8)
```
...
```{r save-todays-data}
 ...
 write.csv(covid,paste0('covid',format(Sys.time(),'%Y%m%d-%H%M'),'.csv'))

 write.csv(covidCZ,paste0('covidCZ',format(Sys.time(),'%Y%m%d-%H%M'),'.csv'))
 
 write.csv(policyCZwiki,paste0('policyCZwiki',format(Sys.time(),'%Y%m%d-%H%M'),'.csv'))

 write.csv(policyCZzpl,paste0('policyCZzakonyprolidi',format(Sys.time(),'%Y%m%d-%H%M'),'.csv'))
