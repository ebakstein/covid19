...
pl3<-pl2 + ggtitle('COVID19 trends: Czech Rep. vs lagged comparison countries') + ylab('total cases') + scale_x_date(breaks=max(covidCZjhu$Date)+days(seq(-14,13,7)),limits=c(max(covidCZjhu$Date)-days(14),max(covidCZjhu$Date)+days(13))) +geom_vline(xintercept =max(covidCZjhu$Date),col='orange' )+ geom_dl(aes(label = country), method = list(dl.combine("last.points"), cex = 0.6))
